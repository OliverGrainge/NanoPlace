...
def analyse_cossim(
    ...
    """
    print(
        f"Analyzing cosine similarity distribution for {len(query_descriptors)} query descriptors and {len(database_descriptors)} database descriptors"
    )
    # Get matches and scores
    matches, scores = match_cosine(
    ...
    fig.suptitle(
        "Cosine Similarity Distribution Analysis", fontsize=16, fontweight="bold"
    )
    ...
    ax3.plot(
        incorrect_sorted, incorrect_cdf, label="Incorrect", color="red", linewidth=2
    )
    ...
    print(
        f"Accuracy: {len(correct_scores)/(len(correct_scores)+len(incorrect_scores))*100:.2f}%"
    )
    ...
    print(
        f"Similarity difference: {np.mean(correct_scores) - np.mean(incorrect_scores):.4f}"
    )
    print("=" * 25)
