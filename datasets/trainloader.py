...
class NanoPlaceDataset(Dataset):
    ...
    def _validate_config(self):
        ...
        missing_cols = [col for col in required_columns if col not in self.config.columns]
        if missing_cols:
    ...
    def _filter_classes_with_min_images(self):
        ...
        original_num_classes = self.config.attrs.get("num_classes", self.config["class_id"].nunique())
        
        # Count images per class
        class_counts = self.config["class_id"].value_counts()
        valid_classes = class_counts[class_counts >= self.images_per_place].index.tolist()
        ...
        old_to_new_mapping = {old_id: new_id for new_id, old_id in enumerate(sorted_classes)}
        ...
        if removed_classes > 0:
            print(f"Removed {removed_classes} classes with <{self.images_per_place} images")
    ...
    def _create_class_to_indices_mapping(self):
        ...
        for class_id, indices in self.class_to_indices.items():
            if len(indices) < self.images_per_place:
                raise ValueError(f"Class {class_id} has only {len(indices)} images, need {self.images_per_place}")
    ...
    def __getitem__(self, class_idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if not (0 <= class_idx < self.num_classes):
            raise IndexError(f"Class index {class_idx} out of range [0, {self.num_classes})")
...
if __name__ == "__main__":
    ...
    print(f"\nBatch shapes:")
    print(f"Images: {batch[0].shape}")  # Should be [batch_size * images_per_place, C, H, W]
