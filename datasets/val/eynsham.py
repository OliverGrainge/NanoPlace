...
class Eynsham(ValDataset): 
    def __init__(self, transform: Optional[transforms.Compose] = None): 
        config = yaml.load(open("config.yaml"), Loader=yaml.FullLoader)
        queries_dir = os.path.join(config["datasets"]["eynsham"], "images/test/queries")
        database_dir = os.path.join(config["datasets"]["eynsham"], "images/test/database")
