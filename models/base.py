...
import faiss 
from eval import match_cosine, compute_recall
import numpy as np 

class NanoPlaceModel(pl.LightningModule): 
    def __init__(self, model_name: str, descriptor_dim: int = 512, loss_name: str = "multisimilarity", miner_name: str = "multisimilarity", cache_dtype: str = "float16"):
        super().__init__() 
    ...
    def on_validation_start(self): 
        ...
        for idx, val_loader in enumerate(self.trainer.val_dataloaders): 
            ...
            self.cache[idx] = np.zeros((len(val_loader.dataset), self.descriptor_dim), dtype=self.cache_dtype)

    def validation_step(self, batch, batch_idx, dataloader_idx=0): 
        images, idxs = batch 
        features = self(images) 
        self.cache[dataloader_idx][idxs.cpu()] = features.cpu().numpy().astype(self.cache_dtype)
    ...
    def _compute_recall(self, dataset_idx: int, k=1): 
        ...
        recall = compute_recall(matches, self.trainer.val_dataloaders[dataset_idx].dataset.ground_truth, k)
