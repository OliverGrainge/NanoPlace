...
def main(args): 
    ...
    descriptors = compute_descriptors(model, dataset, desc_dtype=_desc_dtype(args.desc_dtype), batch_size=args.batch_size, num_workers=args.num_workers)
    query_descriptors = descriptors[:dataset.queries_num] 
    database_descriptors = descriptors[dataset.queries_num:] 
    ground_truth = dataset.ground_truth
    save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"logs/cossim/{args.model_name}/{args.dataset_name}.png")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    analyse_cossim(query_descriptors, database_descriptors, ground_truth, save_path=save_path)
