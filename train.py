...
def load_config_from_argv():
    ...
    if len(sys.argv) < 2:
        raise ValueError(
            "Please provide a path to the config YAML file as the first argument."
        )
...
def load_model(config: DictConfig):
    return NanoPlaceModel(
        config.model_name, config.descriptor_dim, config.loss_name, config.miner_name
    )
...
def load_trainer(config: DictConfig):
    ...
    Logger = pl.loggers.WandbLogger(
        project=logger_config.project, name=logger_config.name
    )
